...
ctree.model.prediction <- function(df.training.cell, df.test.cell){
  model <- ctree(class~ day + time, df.training.cell)
  return(predict(model, df.test.cell))
}
...
accuracy.prediction <- function(prediction, class){
  table.predic <- table(prediction, class)
  cm <- confusionMatrix(prediction, class)
  overall <- cm$overall
  return( overall['Accuracy'] )
...
write.accuracy.to.file = function(dataFrame,cellSize, tolelance){
  write.bus.path = "/Users/chalermpongsomdulyawat/Desktop/Grad_workspace/accuracy_ctree_data/cellsize_"
...
#<-------------------------------------------Main------------------------------------------------>
# for(cell.size in seq(1.5, 1, by = -0.25)){
#print(cell.size)
#tolelance = 20
cell.size = 1
  ...
  for(tolelance in seq(20, 40, by = 5)){
    ...
    df.training <- select.trianing(df)  # 1 week
    df.test <- select.test(df)  # 3 week
    ...
    #cell.index = 50
      ...
      df.test.cell$prediction <- ctree.model.prediction(df.training.cell, df.test.cell)
      accuracy <- accuracy.prediction(df.test.cell$prediction, df.test.cell$class)
      table.predic <- table(df.test.cell$prediction, df.test.cell$class)
      
      
      df.accuracy[cell.index,]$cell = cell.number
      df.accuracy[cell.index,]$ctree_accuracy = accuracy
