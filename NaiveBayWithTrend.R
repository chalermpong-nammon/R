...
plot.graph.train.and.test <- function(cell.number, prediction){
...
make.class <- function(ridership, tolerance){
  return (cut(ridership, breaks = seq(min(ridership)-tolelance, max(ridership), by = tolelance)) )
}
...
get.start.line.of.test <- function(df){
  return(startline.of.test = head(which(df$timeslot == "2010-01-25 00:30:00"), n =1));
}
...
map.ma.data.to.naive.bayes <- function(df.test.cell, df.cell){
  endline.of.trainning.ma = head(which(df.cell$timeslot == "2010-01-24 23:59:59"), n =1)
...
#   df$previous1 <- make.class(0, tolerance)
#   df$previous2 <- make.class(0, tolerance)
#   df$previous3 <- make.class(0, tolerance)
  
  startline.of.test = head(which(df$timeslot == "2010-01-25 00:30:00"), n =1)
  ...
  for(cell.index in 1:length(cell)){
    ...
    df.training.cell$previous1 <- make.class(0, tolerance)
    df.training.cell$previous2 <- as.factor(make.class(0, tolerance))
    df.training.cell$previous3 <- as.factor(make.class(0, tolerance))
    for(x in 4:length(df.training.cell)){
      df.training.cell[x,]$previous1 <- df.training.cell[x-1,]$class
      # df.training.cell[x,]$previous2 <- df.training.cell[x-2,]$class
      # df.training.cell[x,]$previous3 <- df.training.cell[x-3,]$class
    }
    ...
    plot.graph.train.and.test(cell.number, df.test.cell$prediction);
